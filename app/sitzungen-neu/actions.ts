...
export async function createMeetingV3Action(formData: FormData) {
  ...
  if (!title || !startsAt || (meetingType==="custom" && !customTypeLabel)) {
    redirect("/sitzungen-neu?error=missing");
  }
  ...
  if (!id) redirect("/sitzungen-neu?error=create");
  ...
  revalidatePath("/");
  redirect(`/sitzungen-neu/${id}?created=1`);
}
...
export async function updateMeetingV3BasicsAction(formData: FormData) {
  ...
  if (!meetingId || !title || !startsAt) {
    redirect(`/sitzungen-neu/${meetingId}?error=missing`);
  }
  ...
  if (!rows.length) redirect(`/sitzungen-neu/${meetingId}?error=locked`);
  ...
  revalidatePath("/sitzungen-neu");
  redirect(`/sitzungen-neu/${meetingId}?saved=1`);
}
...
export async function updateMeetingV3InvitationAction(formData: FormData) {
  ...
  if (!meetingId || !invitedAt || !invitationMethod || invitationTimely==null || agendaSent==null) {
    redirect(`/sitzungen-neu/${meetingId}?error=invitation`);
  }
  ...
  revalidatePath(`/sitzungen-neu/${meetingId}`);
  redirect(`/sitzungen-neu/${meetingId}?invitation=1`);
}
...
export async function updateMeetingV3OfficersAction(formData: FormData) {
  ...
  if (!meetingId || !chairMemberId || !minuteTakerMemberId) {
    redirect(`/sitzungen-neu/${meetingId}?error=officers`);
  }
  ...
  if (!rows.length) redirect(`/sitzungen-neu/${meetingId}?error=officers`);
  ...
  revalidatePath(`/sitzungen-neu/${meetingId}`);
  redirect(`/sitzungen-neu/${meetingId}?officers=1`);
}
...
export async function addMeetingV3AgendaAction(formData: FormData) {
  ...
  if (!meetingId || !title) redirect(`/sitzungen-neu/${meetingId}?error=agenda`);
  ...
  revalidatePath(`/sitzungen-neu/${meetingId}`);
  redirect(`/sitzungen-neu/${meetingId}?agenda=1`);
}
...
export async function markMeetingV3ReadyAction(formData: FormData) {
  ...
  if (!meeting) redirect(`/sitzungen-neu/${meetingId}?error=locked`);
  ...
  if (!readiness.ready) {
    redirect(`/sitzungen-neu/${meetingId}?error=not_ready`);
  }
  ...
  revalidatePath(`/sitzungen-neu/${meetingId}`);
  revalidatePath("/sitzungen-neu");
  redirect(`/sitzungen-neu/${meetingId}?ready=1`);
}
...
export async function reopenMeetingV3PreparationAction(formData: FormData) {
  ...
  revalidatePath(`/sitzungen-neu/${meetingId}`);
  revalidatePath("/sitzungen-neu");
  redirect(`/sitzungen-neu/${meetingId}?preparation=1`);
}
